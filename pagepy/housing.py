...
def process_spreadsheet_value(tab, **kwargs):
    ...
    print(tab)
    tab['First name'] = [r for r in tab['Full Name']]
    tab['Check-in date'] = [Time(r, format='iso', out_subfmt='date').to_value(format='iso', subfmt='date') for r in tab['Check-in date']]
    tab['Check-out date'] = [Time(r, format='iso', out_subfmt='date').to_value(format='iso', subfmt='date') for r in tab['Check-out date']]
    tab['Room'] = [r.split('($')[0] for r in tab['Rank ordered list of housing choice [First choice]']]
    print(tab['Room'])
    tab['Roommate'] = [r for r in tab['Roommate']]
    print(tab['Roommate'])
    ...
    for i, f in enumerate(tab['First name']):
        if " for " in f:
            a, b = f.split(' for ')
            tab['First name'][i] = a
            team[i] = ' for ' + b
